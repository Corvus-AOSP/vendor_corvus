...
func init() {
	android.RegisterModuleType("corvus_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = corvusExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = corvusExpandVariables(ctx, dep_file)
	...
	cmd := corvusExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestPath := android.PathForOutput(ctx, "generator.sbox.textproto")
