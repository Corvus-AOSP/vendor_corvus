...
function showHelpAndExit {
        ...
        echo -e "${CLR_BLD_BLU}  -v, --variant         CORVUS variant - Can Official or Unofficial${CLR_RST}"
...
while true; do
    case "$1" in
        ...
        -v|--variant|v|variant) RAVEN_LAIR="$2"; shift;;
...
if [ ! -d "$DIR_ROOT/vendor/corvus" ]; then
        echo -e "${CLR_BLD_RED}error: insane root directory ($DIR_ROOT)${CLR_RST}"
        exit 1
fi

# Setup CORVUS variant if specified
if [ $CORVUS_VARIANT ]; then
    CORVUS_VARIANT=`echo $CORVUS_VARIANT |  tr "[:upper:]" "[:lower:]"`
    if [ "${CPRVUS_VARIANT}" = "Official" ]; then
        export RAVEN_LAIR=Official
    elif [ "${CORVUS_VARIANT}" = "Unofficial" ]; then
        export RAVEN_LAIR = Unofficial
    else
        echo -e "${CLR_BLD_RED} Unknown Corvus variant - use Official or Unofficial${CLR_RST}"
        exit 1
    fi
fi
...
CORVUS_DISPLAY_VERSION="$(cat $DIR_ROOT/vendor/corvua/config/branding.mk | grep 'CORVUS_VERSION_BASE := *' | sed 's/.*= //') \
$(cat $DIR_ROOT/vendor/corvus/config/branding.mk | grep 'CORVUS_CODENAME := *' | sed 's/.*= //')"
...
echo -e "${CLR_BLD_GRN}Building CORVUS $CORVUS_DISPLAY_VERSION for $DEVICE${CLR_RST}"
...
CORVUS_VERSION="$(get_build_var CORVUS_VERSION)"
checkExit
echo -e ""
...
# Build signed rom package if specified
    ...
    sign_target_files_apks -o -d $KEY_MAPPINGS \
        "$OUT"/obj/PACKAGING/target_files_intermediates/corvus_$DEVICE-target_files-$FILE_NAME_TAG.zip \
        corvus-$DEVICE-signed-target_files-$FILE_NAME_TAG.zip
...
# Build rom package
    ...
    cp -f $OUT/corvus_$DEVICE-ota-$FILE_NAME_TAG.zip $OUT/$CORVUS_VERSION.zip
    echo "Package Complete: $OUT/$CORVUS_VERSION.zip"
fi
echo -e ""
